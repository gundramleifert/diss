...
public class ApplyATR extends FolderOrganizer implements Callable<Boolean> {
...
//    TextAligner textAligner = null;
    private static Logger LOG = Logger.getLogger(ApplyATR.class);

    public ApplyATR(String mainset, String subset, String net, String configName) {
        super(mainset, subset, net, configName);
        addReflection(this, ApplyATR.class);
    }
...
//        String nameATR = new File(atr).getName();
        ...
        File folderOut = getFolderResultModelTranscript();
        folderOut.mkdirs();
                ...
                for (XPage page : file.getPages()) {
                    for (XRegion region : page.getRegions()) {
                        for (XLine line : region.getLines()) {
                            ConfMat confMat = confMats.get(idxLine);
                            double sum = 0;
                            int cnt = 0;
                            double[] bestPathLLHProbs = confMat.getBestPathLLHProbs();
                            int[] idxs = confMat.getMaxIdxs();
                            for (int i = 0; i < idxs.length; i++) {
                                if (idxs[i] != 0) {
                                    cnt++;
                                    sum += bestPathLLHProbs[i];
                                }
                            }
                            line.setTextConf(Math.exp(sum / cnt));
                            line.setText(confMat.getString(confMat.getBestPath()));
                            idxLine++;
                        }
                    }
                }
